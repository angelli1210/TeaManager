...
const emptyForm = { supplierId: '', supplierName: '', country: '', contactPerson: '', email: '', phone: '', address: '' };
...
export default function SuppliersPage() {
  ...
  const openEdit = (s) => {
    setForm({ supplierId: s.supplierId, supplierName: s.supplierName, country: s.country, contactPerson: s.contactPerson, email: s.email, phone: s.phone || '', address: s.address || '' });
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map(s => (
                <tr key={s.supplierId} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-gray-500">{s.contactPerson}</td>
              ...
              {filtered.length === 0 && <tr><td colSpan={7} className="px-5 py-10 text-center text-gray-400 text-sm">No suppliers found.</td></tr>}
            </tbody>
          </table>
        )}
      </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-7 mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { label:'Contact Person', field:'contactPerson', placeholder:'e.g. Wang Fang', required:true },
                { label:'Email', field:'email', type:'email', placeholder:'email@supplier.com', required:true },
                { label:'Phone', field:'phone', placeholder:'+86-21-5555' },
                { label:'Address', field:'address', placeholder:'Street address', full:true },
